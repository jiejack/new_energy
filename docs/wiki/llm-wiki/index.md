# 新能源监控系统知识库

## 项目概述

本知识库是为新能源监控系统项目服务的专业知识库，采用LLM-wiki架构模式，实现知识的持续积累、自动交叉引用和主动检测矛盾。经过500轮迭代，知识库已形成完整的知识体系。

## 知识库结构

### 核心知识文档
- [500轮迭代知识树图](knowledge-tree-500.md) - 完整的知识树结构
- [500轮迭代项目技能总结](../../skills/project-skills-500.md) - 可复用项目技能
- [500轮迭代错误处理记录](../../error-handling/iteration-errors-500.md) - 问题与解决方案

### 实体 (Entities)
- [Go 编程语言](entities/go-language.md) - Go 1.24+ 编程语言
- [Gin Web 框架](entities/gin-framework.md) - Go Web 框架
- [Vue 3 前端框架](entities/vue3-framework.md) - Vue.js 3.x 前端框架
- [Excelize 库](entities/excelize-library.md) - Go Excel 文档操作库
- [TypeScript 编程语言](entities/typescript-language.md) - TypeScript 5.0+ 编程语言

### 概念 (Concepts)
- [前后端分离架构](concepts/frontend-backend-separation.md)
- [分层架构设计](concepts/layered-architecture.md)
- [依赖注入](concepts/dependency-injection.md)
- [微服务架构](concepts/microservices.md)

### 技术 (Technologies)
- [RESTful API 设计](technologies/restful-api.md)
- [WebSocket 实时通信](technologies/websocket.md)
- [Excel 文件处理](technologies/excel-processing.md)
- [数据库设计](technologies/database-design.md)

### 最佳实践 (Best Practices)
- [代码规范](best-practices/code-standards.md)
- [Git 工作流程](best-practices/git-workflow.md)
- [测试策略](best-practices/testing-strategy.md)
- [性能优化](best-practices/performance-optimization.md)

### 需求 (Requirements)
- [告警规则管理](requirements/alarm-rule-management.md)
- [统计报表功能](requirements/report-function.md)
- [前端优化设计](requirements/frontend-optimization.md)
- [能效分析功能](requirements/energy-efficiency-analysis.md)
- [碳排放监测功能](requirements/carbon-emission-monitoring.md)

### 来源摘要 (Sources)
- [Andrej Karpathy LLM-wiki 概念](sources/karpathy-llm-wiki.md)

### 领域知识 (Domains)
- [新能源行业知识](domains/new-energy-industry.md) - 新能源行业概述、技术、标准等

## 知识库里程碑

### 第1-100轮迭代 - 基础建设
- 建立知识库基础结构
- 添加核心技术实体文档
- 完成业务需求分析
- 建立领域知识体系

### 第101-300轮迭代 - 知识扩展
- 扩展技术栈知识
- 添加架构设计知识
- 完善安全知识体系
- 建立AI/ML知识分支

### 第301-500轮迭代 - 知识成熟
- 完成知识树构建
- 提炼项目技能总结
- 记录错误处理经验
- 建立知识成长路径

## 操作日志

- [2026-04-15] init | 初始化知识库 - 创建知识库基础结构和索引文件
- [2026-04-15] add | 添加技术实体页面 - Go、Gin、Vue3、Excelize、TypeScript
- [2026-04-15] add | 添加业务需求文档 - 能效分析、碳排放监测
- [2026-04-15] add | 添加领域知识 - 新能源行业知识
- [2026-04-15] milestone | 500轮迭代完成 - 添加知识树图、技能总结、错误记录
- [2026-04-15] update | 更新索引 - 完善知识库结构，添加里程碑记录
